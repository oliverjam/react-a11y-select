...
export default class Select extends React.Component {
  ...
  open = () => {
    ...
    this.setState(
      {
        expanded: true,
        status: `${options.length} options available. Use up and down to choose, or escape to close.`,
      },
      () => {
        this.list.focus();
      }
  ...
  updateSelected = () => {
    ...
    this.setState({
      ...
      status: `${options[highlightedIndex]} selected.`,
    });
    this.close();
  };
  ...
  updateHighlighted = optionKey => {
    const { options } = this.props;
    this.setState({
      highlightedIndex: optionKey,
      status: `${options[optionKey]}, ${optionKey + 1} of ${options.length}`,
    });
  };
  ...
  render() {
    ...
    const { expanded, selectedIndex, highlightedIndex, status } = this.state;
    return (
      <Wrapper onKeyDown={this.handleKeyDown}>
        ...
        <LiveRegion aria-atomic="true" aria-live="assertive" role="status">
          {status}
        </LiveRegion>
      </Wrapper>
    );
